...
let sThresh = 8;  // Threshold for similarity of semiminor axes, in pixels
let binThresh = 0.15;  // Threshold for a bin to be counted as an ellipse, in votes / unit circumference
...
function preload() {
  ...
  ellipsesImage = loadImage("data/ellipses2.png");
  ...
  //  for (let p2 = 0; p2 < poi.length; p2++) {
      ...
      if (max(min(angbtwn211, angbtwn121), min(angbtwn212, angbtwn122)) < PI/6) {  // 30 degrees = PI/6 radians
        ...
        for (let p = 0; p < poi.length; p++) {
          ...
          if (dotcenter > cos(PI/6)) {  // about 30 degrees
